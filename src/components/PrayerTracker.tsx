...
import { PRAYERS, getStudents, savePrayerEntry, getPrayerEntryAsync, CLASSES } from "@/lib/prayerData";
import { useToast } from "@/hooks/use-toast";
import { Sunrise, Sun, CloudSun, Sunset, Moon, Loader2 } from "lucide-react";
...
const PrayerTracker = ({ onSubmit }: { onSubmit: () => void }) => {
  ...
  const loadExisting = async (studentName: string, d: string) => {
    const entry = await getPrayerEntryAsync(d, studentName);
    if (entry) {
      setPrayers({
