...
// Firebase key-safe encoding (Firebase doesn't allow . $ # [ ] /)
...
export async function savePrayerEntry(entry: PrayerEntry): Promise<void> {
  const encodedName = encodeKey(entry.name);
  const entryRef = ref(database, `prayerData/${entry.date}/${encodedName}`);
  await set(entryRef, entry);
}

export async function getPrayerDataAsync(): Promise<Record<string, Record<string, PrayerEntry>>> {
  const snapshot = await get(ref(database, 'prayerData'));
  if (!snapshot.exists()) return {};
  const raw = snapshot.val();
  // Reconstruct: dates -> names -> entries
  const result: Record<string, Record<string, PrayerEntry>> = {};
  for (const date in raw) {
    result[date] = {};
    for (const encodedName in raw[date]) {
      const entry = raw[date][encodedName] as PrayerEntry;
      result[date][entry.name] = entry;
    }
  }
  return result;
}

export async function getPrayerEntryAsync(date: string, name: string): Promise<PrayerEntry | null> {
  const encodedName = encodeKey(name);
  const snapshot = await get(child(ref(database), `prayerData/${date}/${encodedName}`));
  return snapshot.exists() ? (snapshot.val() as PrayerEntry) : null;
}

export async function deletePrayerEntry(date: string, name: string): Promise<void> {
  const encodedName = encodeKey(name);
  await remove(ref(database, `prayerData/${date}/${encodedName}`));
}
...
// Legacy sync wrappers (for backward compat) - prefer async versions
